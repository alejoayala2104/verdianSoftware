...
public class ClientesControlador {	 
	...
	//M�todo que fuerza a los campos de c�dula y tel�fono a solo recibir entradas num�ricas.
	//Tambi�n hace que no se pueda digitar del n�mero 0 de primero.		
	@FXML
	...
	public void registrarCliente(ActionEvent event)throws SQLException{
		...
		if(cedulaTF.getText().isEmpty() || nombresTF.getText().isEmpty() || telefonoTF.getText().isEmpty() ||
			...
			mostrarAlerta(AlertType.ERROR, "Error al ingresar", null, "Todos los datos son obligatorios. Por favor rellenar los campos vac�os.");			
			return;
		}
		
		//En caso de que el cliente exista, muestra la alerta y sale del metodo registrarCliente().
		if(validarCliente(cedulaTF)) {			
			mostrarAlerta(AlertType.ERROR, "Cliente existente", null, "El cliente ya est� registrado en el sistema.");			
			return;
		}
		...
		mostrarAlerta(AlertType.INFORMATION, "Registro creado", "Cliente registrado con �xito", null);
		limpiarTextFields();
	...
	public void consultarCliente(ActionEvent event) throws SQLException{	
		...
		//Valida que el campo de c�dula no est� vac�o.
		if(buscarCedulaTF.getText().isEmpty()) {	
			limpiarTextFields();
			mostrarAlerta(AlertType.ERROR, "Error", "Campo vac�o", "Por favor ingrese un valor en la casilla de b�squeda.");			
			return;
		}
		...
		//Si no, dar aviso de que no se encontr� el cliente, limpiar los textfield y salir del controlador consultarCliente().
		else {
			limpiarTextFields();
			mostrarAlerta(AlertType.ERROR, "Cliente no encontrado", null, "No se encontraron registros con la c�dula ingresada.");
			return;
		}
	...
	public void actualizarCliente(ActionEvent event) throws SQLException{
		...
		//Verificar que el campo de cedula no est� vac�o.
	...
	public void actualizarClienteGuardar(ActionEvent event) throws SQLException{
		...
		//Verifica que la c�dula que est� ingresando no es de alg�n otro cliente ya existente.
		if(actCedulaTF.getText().equals(cedulaBuscada)==false && validarCliente(actCedulaTF)) {
			limpiarTextFields();
			mostrarAlerta(AlertType.ERROR, "Cliente existente", "No se puede actualizar la c�dula", "El cliente con dicha c�dula ya existe.");
			return;
		}
		...
		mostrarAlerta(AlertType.INFORMATION, "Registro actualizado", "Cambios guardados con �xito", null);
		limpiarTextFields();
