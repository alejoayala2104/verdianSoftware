...
public class TransControlador implements Initializable {
    ...
    @FXML
    private HBox hbxGarantiasAnadidas;
    @FXML
    private TextArea taGarantiasAnadidas;
    ObservableList<Garantia> listaGarantias;
    ArrayList<Garantia> listaGarantiasAnadidas = new ArrayList<Garantia>();     
   ...
   //Registrar transacción: Información de Pago Inversión
    ...
    public void resetearTodo() {
		...
		this.listaGarantiasAnadidas = new ArrayList<Garantia>();  	    
    ...
    public void generarSimulacion(ActionEvent event) {
    	...
    	for(int i=0; i<numCuotas;i++) {
    		int noCuota = i+1;
    		LocalDate fechaPago = fechaIniciacion.plusMonths((long) i+1);
    ...
    public void continuarASoportes(ActionEvent event) throws SQLException {
    	...
    	if(objTransaccion.getTipoTrans()=='I') {
    		...
    		taGarantiasAnadidas.setVisible(false);
    		hbxGarantiasAnadidas.setVisible(false);
    		lblRegTransClieSinGarantias.setVisible(true);
    		//Para limpiar el TextArea
    		cancelarAnadidasRegTrans(event);
    ...
    public void mostrarGarantiasRegTrans() {
		...
		try {
    		if(garantias.next()) {
    	    	...
    	    	taGarantiasAnadidas.setVisible(true);
    			lblRegTransClieSinGarantias.setVisible(false);
    			hbxGarantiasAnadidas.setVisible(true);
    ...
    @FXML
    public void anadirGarantiaRegTrans(ActionEvent event) {
    	
    	if(tableRegTransGarantias.getSelectionModel().isEmpty()) {
    		controlGeneral.mostrarAlerta(AlertType.ERROR, "Garantía inválida", "Garantía no seleccionada", "Por favor seleccione una garantía para anadirla.");
    		return;
    	}
    	
    	Garantia garantiaAnadida = new Garantia();
    	garantiaAnadida = tableRegTransGarantias.getSelectionModel().getSelectedItem();
    	for(Garantia gar:listaGarantiasAnadidas) {
    		if(gar.equals(garantiaAnadida))
    			return;
    	}
    	listaGarantiasAnadidas.add(garantiaAnadida);
    	taGarantiasAnadidas.appendText("\n"+garantiaAnadida.mostrarGarantia());
    }
    
    @FXML
    public void anadirCuentaBancariaRegTrans(ActionEvent event) {
    	...
    	if(tableRegTransCuentasBanc.getSelectionModel().isEmpty()) {
    		controlGeneral.mostrarAlerta(AlertType.ERROR, "Cuenta bancaria inválida", "Cuenta bancaria no seleccionada",
    				"Por favor seleccione una cuenta bancaria para anadirla.");
    		return;
    	}
    ...
    @FXML
    public void cancelarAnadidasRegTrans(ActionEvent event) {
    	listaGarantiasAnadidas = new ArrayList<>();
    	taGarantiasAnadidas.clear();
    }
    ...
    public void continuarAFiadorRegTrans(ActionEvent event) throws SQLException {    	
    	...
    	if(listaGarantiasAnadidas.isEmpty()) {
    		controlGeneral.mostrarAlerta(AlertType.INFORMATION, "Garantías pendientes", "No se anadieron garantías", "El estado de la solicitud será INCOMPLETA.");
    		objTransaccion.setEstadoSolicitud("INCOMPLETA");
    	}
    ...
    public void finalizarRegTransPrestamo(ActionEvent event) throws SQLException {
    	...
    	if(!(listaGarantiasAnadidas.isEmpty()) && fiadorOK)
    	...
    	if(!(listaGarantiasAnadidas.isEmpty())) {
    		...
    		controlGeneral.mostrarAlerta(AlertType.INFORMATION, "Garantias anadidas", "Garantías asociadas al préstamo exitosamente.", null);
    	}
...
//    	System.out.println(transSelected.toString());
    ...
    @FXML
    private HBox hbxActTransGarantiasAnadidas;
    ...
    @FXML
    private ListView<Garantia> lsvActTransGarantiasAnadidas;
    ...
    public void actTransContinuarAModificacion(ActionEvent event) throws SQLException{
    	...
    	if(this.objTransaccion.getTipoTrans()=='P') {    		
	    	...
	    	this.lsvActTransGarantiasAnadidas.setItems(this.listaGarantiasTrans);
    	}
    	else {
    ...
    public void mostrarGarantiasActTrans() {
		...
		try {
    		if(garantias.next()) {
    			...
    			lsvActTransGarantiasAnadidas.setVisible(true);
    			lblActTransClienteSinGarantias.setVisible(false);
    			hbxActTransGarantiasAnadidas.setVisible(true);
    ...
    public ObservableList<Garantia> buscarGarantiasTrans(int codigoTrans) throws SQLException{
    	
    	ObservableList<Garantia> listaGarantiasTrans = FXCollections.observableArrayList();
    	codigoTrans = this.objTransaccion.getCodTrans();
    	String buscarGarantia = "select * from transacciones join prestamos on codtrans=codprestamo "
    			+ "natural join garantias_prestamo natural join garantias where codtrans="+codigoTrans+";";
    ...
    @FXML
    public void actTransAnadirGarantiaListView(ActionEvent event) throws SQLException {
    	if(tblActTransGarantias.getSelectionModel().getSelectedItem()==null) {
    		controlGeneral.mostrarAlerta(AlertType.ERROR, "ERROR: Garantia no seleccionada", "Garantía no seleccionada",
    				"Por favor seleccione una garantía de la tabla y presione Anadir");
    		return;
    	}
    	
    	Garantia garantiaAnadida = tblActTransGarantias.getSelectionModel().getSelectedItem();    	
    	for(int i=0;i<this.listaGarantiasTrans.size();i++) {    		
    		if(garantiaAnadida.getCodGarantia()==listaGarantiasTrans.get(i).getCodGarantia()) {
    			return;
    		}
    	}
    	this.listaGarantiasTrans.add(garantiaAnadida);
		this.lsvActTransGarantiasAnadidas.setItems(this.listaGarantiasTrans);    	
    }
    
    @FXML
    public void actTransBorrarGarantiaListView(ActionEvent event) {
    	if(this.lsvActTransGarantiasAnadidas.getSelectionModel().getSelectedItem()==null) {
    		controlGeneral.mostrarAlerta(AlertType.ERROR, "ERROR: Garantia no seleccionada", "Garantía no seleccionada",
    				"Por favor seleccione una garantía de la lista para borrarla.");
    		return;
    	}
    	
    	Garantia garantiaABorrar = this.lsvActTransGarantiasAnadidas.getSelectionModel().getSelectedItem();
	...
	public void initialize(URL location, ResourceBundle resources) {
		...
		//Se esconden los VBox que contienen las info de garantias. La tabla y las que serán anadidas.
		vbxRegTransTablaGarantias.setVisible(false);
		taGarantiasAnadidas.setVisible(false);
		hbxGarantiasAnadidas.setVisible(false);
